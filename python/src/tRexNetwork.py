...
def standard_dqn(flatten, dense_initialization, num_actions, lstm_dropout):
    # original network
...
def duel_drqn(flatten, dense_initialization, num_actions, lstm_dropout, lstm_initialization='glorot_normal'):

    def add_lstm(flatten):
        flatten_expand = expand_dims(flatten, -1)
        lstm_out = LSTM(units=512, kernel_initializer=lstm_initialization, dropout=lstm_dropout)(flatten_expand)
        return lstm_out

    # implementation of the duel network architecture (see http://proceedings.mlr.press/v48/wangf16.pdf)
    # value stream
    lstm_value = Lambda(add_lstm)(flatten)
    out_value = Dense(1, kernel_initializer=dense_initialization)(lstm_value)
    # advantage stream
    lstm_advantage = Lambda(add_lstm)(flatten)
    out_std_advantage = Dense(num_actions, kernel_initializer=dense_initialization)(lstm_advantage)
